...
v1.22.0 是一个**次要功能版本**，带来长期期待的 Schema 一键更新工作流、繁体中文作为第 10 种语言，以及改进的摄取状态栏。

- **📝 Schema 一键应用（Issue #97）。** LLM 生成的 Schema 优化建议现在在 IDE 风格的双栏 diff Modal 中显示，提供"应用"/"取消"/"打开文件"按钮。应用建议时会自动备份当前 Schema（轮转保留最多 3 份），然后写入新内容。"更新 Schema"现在通过 Lint Modal 访问——命令面板入口已移除，强制单一入口点。
- **🏷️ Schema 动态标签同步。** Schema 词表现在是唯一事实来源——活动标签自动注入每次 LLM 调用，消除了 v1.21.0 Phase 1 中"Schema 模板被硬编码覆盖"的 Bug。
...
- **📋 自动摄入不再弹出阻断弹窗 (Issue #204)。** 监视模式的自动摄入现在默认显示短暂通知 (Notice)，不再打开完整摄入报告弹窗。偏好详细报告的用户可在设置 → 自动维护 → 监视模式中切换为 "详细报告"。操作历史面板和 `log.md` 随时可供查阅。
- **🔧 Auto Smart Fix 弹窗 → 短暂通知。** Auto Smart Fix 完成后，简洁的 Notice 带有操作历史面板提示，替代了之前阻断式 FixReportModal。行为由相同的 "自动摄入通知方式" 设置控制。
- **🌐 操作日志现支持 10 语言。** 首次创建 `log.md`（或下次启动后）会获得自动迁移的头部，解释该日志并引导到操作历史面板。每种语言显示对应本地化文本。
- **📅 定时维护：移除"每小时"，新增"每月"。** 现存"每小时"选择会自动回退到"每天"。保留选项：关闭、每天、每周、每月。
- **🧹 死代码清理。** 移除了 `slug.ts` 中冗余的 `console.debug` 和 `main.ts` 中两处无用的 `setDoneCallback` 重置。
