...
v1.20.2 是一个**补丁版本**，修复了对较新 Claude 模型（Opus 4.8、Sonnet 4.6、Fable 5、Mythos 5）的 Anthropic API 兼容性问题。结合 v1.20.0 的 Provider 优先思考控制和 v1.20.1 的 prefill 检测，本版本确保了完整的 Anthropic 支持。

- **🔧 Anthropic 回退修复（PR #151）。** Anthropic Messages API 仅接受 messages 中的 user/assistant 角色——system 必须是顶层字段。之前的回退路径将 `{role: 'system'}` 注入 messages，导致第二次 400 错误。由 @Indexed-Apogrypha 修复。
- **🧠 Provider 优先的思考控制（v1.20.0）。** 默认不发送任何思考控制字段——由 Provider 自行决定。
- **💭 可折叠思考 UI（v1.20.0）。** 推理内容以折叠面板显示在答案上方。支持 8 种语言。
- **🔧 Anthropic baseUrl 修复（v1.20.0，#141、#134）。** 修复测试连接时的 404 错误。
- **🔧 gpt-5 max_completion_tokens（v1.20.0，#143）。** 使用正确的 token 参数。
- **💬 Query Wiki UX（v1.20.0）。** 尊重 wikiFolder、自动滚动、用户消息右对齐。
- **🔄 自动迁移。** 旧用户的 disableThinking 自动重置为 false。

我们强烈建议所有用户升级到此版本，以获得完整的 Anthropic 兼容性和所有最新功能。
