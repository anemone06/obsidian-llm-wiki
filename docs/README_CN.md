...
本次更新为 **Bug 修复批次**，修复了 Lint 取消失效、维护时粒度限制不生效、推理模型思考 token 泄露等问题，并新增了"删除空洞页面"快捷操作——无破坏性变更，无需重新配置。

**核心修复：**

- **修复 Lint 取消按钮（Issue #94）。** 点击状态栏"点击取消"现在能在修复阶段（死链、空洞、孤立、别名、重复合并）正常生效——AbortSignal 已透传到所有 5 个 fix-runner 函数。所有持久化 Notice 已用 try/finally 包裹，确保取消后自动消失。

- **Lint 尊重提取粒度限制（Issue #96）。** Lint 的 LLM 分析步骤之前不受 extractionGranularity 约束，现在通过 appendGranularityToPrompt 注入粒度指令——"Minimal"用户获取受限分析，"Fine"用户获取完整分析。

- **消除推理模型思考 token 泄露（Issue #99 + #86）。** 会输出前置思考文本或 <think> 块的推理模型不再污染 Wiki 页面。三层防御：(1) API 层统一发送 thinking.type=disabled + 自动 400 回退；(2) JSON 响应在解析前剥离 think token；(3) Markdown 响应丢弃第一个 --- 或 # 前的所有内容。Test Connection 时会探测并提供商支持结果。

- **新增"删除空洞页面"（Issue #103）。** Lint 报告弹窗中新增按钮，与现有的"扩充"按钮并列——一键清除空洞 stub，无需跑完整 Lint 流水线。跳过 reviewed: true 的页面。无需配置。

**从旧版本升级？** 零破坏性变更，零重新配置。既有 Wiki、设置、工作流全部保留。

**强烈建议所有用户升级到该版本**——Lint 取消修复、粒度约束、思考 token 防护显著提升了日常维护的可靠性。
