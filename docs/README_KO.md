...
이 PATCH는 자동 수집 UX 개선, 작업 기록 i18n, 데드 코드 제거를 포함합니다.

- **📋 자동 수집이 더 이상 모달을 차단하지 않음 (Issue #204).** 감시 모드의 자동 수집이 기본적으로 전체 수집 보고서 모달 대신 일시적인 알림(Notice)을 표시합니다. 상세 보고서를 원하는 사용자는 설정→자동 유지보수→감시 모드에서 "모달"로 전환할 수 있습니다. 작업 기록 패널과 `log.md`는 언제든지 확인 가능합니다.
- **🔧 Auto Smart Fix 모달→일시적 알림.** Auto Smart Fix 완료 시 작업 기록 패널 힌트가 포함된 간결한 알림을 표시합니다.
- **🌐 작업 기록이 i18n 지원 (10개 언어).** `log.md` 최초 생성 시(또는 기존 Wiki의 다음 시작 시) 로그를 설명하고 작업 기록 패널로 안내하는 헤더가 자동 생성됩니다. 각 언어에 맞는 텍스트를 표시합니다.
- **📅 정기 Lint: "매시간" 제거, "매월" 추가.** 기존 "매시간" 설정은 자동으로 "매일"로 대체됩니다. 남은 옵션: 끄기, 매일, 매주, 매월.
- **🧹 데드 코드 제거.** `slug.ts`의 불필요한 `console.debug`와 `main.ts`의 불필요한 `setDoneCallback` 리셋을 제거했습니다.
...
- **🤫 Auto Ingest 가 이제 `autoIngestNotificationLevel: notice` 를 존중합니다 (Issue #204).** v1.22.2 에서 추가된 `onAutoIngestDone` 헬퍼(Notice 경로)가 Watch Mode 자동 ingest 흐름에 연결되지 않아 — 모든 자동 ingest 완료가 항상 `IngestReportModal` 을 여는 `onIngestDone` 을 거쳤기 때문에 Notice 설정이 무용지물이었습니다. v1.22.6 는 `IngestReport` 와 `IngestOptions` 에 `trigger?: 'auto' | 'manual'` 필드를 추가하고, `WikiEngine.ingestSource` → `onDone` 을 통해 전파하여 `trigger='auto'` 를 `onAutoIngestDone` 으로 라우팅합니다. 수동 ingest 동작은 변경되지 않습니다.
- **🔇 Auto Smart Fix 완료도 컨텍스트 인지.** 동일한 trigger 패턴을 `runLintWiki` 에 적용 (새로운 세 번째 `trigger` 파라미터, 기본값 `'manual'`). 주기적 자동 lint 가 `trigger='auto'` 를 전달합니다. 완료 디스패치: 수동 → `LintReportModal`; 자동 + `autoSmartFix=true` → Notice + fixAll; 자동 + `autoSmartFix=false` → Notice 만, History 패널 힌트 포함.
- **🛡️ GPT-5 Pro 변형(`gpt-5.x-pro`)이 이제 `/v1/responses` 로 올바르게 라우팅됩니다 (Issue #207 후속).** OpenAI 공식 문서에서 검증됨: "GPT-5 Pro is available in the Responses API only." v1.22.5 의 regex 는 `gpt-5.x` 에는 매치되었지만 `-pro` 접미사를 놓쳐서 — `gpt-5.2-pro` / `5.4-pro` / `5.5-pro` 가 조용히 `/v1/chat/completions` 로 가서 404. regex 를 `^(gpt-5\.[1-9]\d*(?:-pro)?|...)` 로 확장.

업그레이드 권장 — "Auto Ingest Notice" 설정이 마침내 동작하고, 주기적 자동 lint 가 작성 흐름을 더 이상 방해하지 않으며, Pro 모델 변형이 Responses API 를 통해 접근 가능합니다.
