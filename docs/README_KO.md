...
- [⚡ v1.16.2 업데이트 하이라이트](#-v1162-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
이번 릴리스는 **버그 수정 배치**입니다. Lint 취소 기능 수정, 유지보수 시 추출 세분화 적용, 추론 모델의 Thinking 토큰 누출 방지, "빈 스텁 삭제" 기능 추가를 포함합니다. 호환성 깨짐 없음, 재구성 불필요.

**주요 수정 사항：**

- **Lint 취소 기능 수정 (Issue #94).** 상태 표시줄의 "클릭하여 취소"가 이제 수정 단계(데드 링크, 빈 페이지, 고아 페이지, 별칭, 중복)에서도 정상 작동합니다 — AbortSignal이 5개 모든 fix-runner 함수에 전파됩니다. 모든 영구 Notice는 try/finally로 래핑되어 취소 시에도 사라집니다.

- **Lint가 추출 세분화를 준수 (Issue #96).** Lint의 LLM 분석 단계는 이전에 제약이 없었습니다. 이제 extractionGranularity 설정을 준수합니다 — "Minimal" 사용자는 제한된 분석을, "Fine" 사용자는 전체 분석을 받습니다.

- **Thinking 토큰 누출 해결 (Issue #99 + #86).** 실제 출력 앞에 프리앰블 텍스트나 <think> 블록을 출력하는 추론 모델이 더 이상 Wiki 페이지를 오염시키지 않습니다. 3계층 방어: (1) API 계층에서 thinking.type=disabled를 통일 전송 + 자동 400 폴백; (2) JSON 응답은 파싱 전 think 토큰 제거; (3) Markdown 응답은 첫 번째 --- 또는 # 헤더 이전의 모든 내용을 폐기합니다. Test Connection 시 프로바이더 지원 여부를 탐색하여 캐시합니다.

- **"빈 스텁 삭제" 추가 (Issue #103).** Lint 리포트 모달에 기존 "확장" 버튼과 함께 새 버튼이 추가되었습니다 — 한 번의 클릭으로 빈 스텁을 정리하고 전체 Lint 파이프라인을 실행할 필요가 없습니다. reviewed: true 페이지는 건너뜁니다. 설정이 필요 없습니다.

**이전 버전에서 업그레이드？** 호환성 깨짐 없음, 재구성 불필요. 기존 Wiki, 설정, 워크플로우가 모두 유지됩니다.

**모든 사용자에게 이 버전으로의 업그레이드를 강력히 권장합니다** — Lint 취소 수정, 세분화 적용, Thinking 토큰 보호로 일상적인 유지보수의 신뢰성이 크게 향상됩니다.
---
## ✨ 주요 기능
