...
- [⚡ v1.20.2 업데이트 하이라이트](#-v1202-업데이트-하이라이트)
- [✨ 주요 기능](#-주요-기능)
  - [📊 지식 품질](#-지식-품질)
  - [🛠️ 유지 관리](#️-유지-관리)
  - [💬 조회 및 피드백](#-조회-및-피드백)
...
v1.20.2는 최신 Claude 모델(Opus 4.8, Sonnet 4.6, Fable 5, Mythos 5)에 대한 Anthropic API 호환성을 수정한 **패치 릴리스**입니다. v1.20.0의 프로바이더 우선 사고 제어와 v1.20.1의 prefill 감지와 결합하여, 이번 릴리스는 완전한 Anthropic 지원을 보장합니다.

- **🔧 Anthropic 폴백 수정 (PR #151).** Anthropic Messages API는 messages 내의 user/assistant 역할만 허용합니다——system은 최상위 수준이어야 합니다. 이전 폴백 경로는 `{role: 'system'}`을 messages에 주입하여 두 번째 400 오류를 유발했습니다. @Indexed-Apogrypha가 수정했습니다.
- **🧠 프로바이더 우선 사고 제어 (v1.20.0).** 기본적으로 사고 제어 필드를 전송하지 않습니다. 프로바이더가 결정합니다.
- **💭 접을 수 있는 사고 UI (v1.20.0).** 추론 콘텐츠를 답변 위의 접을 수 있는 패널로 표시. 8개 언어 지원.
- **🔧 Anthropic baseUrl 수정 (v1.20.0, #141, #134).** 테스트 연결 시 404 오류 수정.
- **🔧 gpt-5 max_completion_tokens (v1.20.0, #143).** 올바른 토큰 파라미터 사용.
- **💬 Query Wiki UX (v1.20.0).** wikiFolder 존중, 자동 스크롤, 사용자 메시지 우측 정렬.
- **🔄 자동 마이그레이션.** 이전 사용자의 disableThinking이 자동으로 false로 재설정.

모든 사용자에게 완전한 Anthropic 호환성과 최신 기능을 위해 이 버전으로의 업그레이드를 강력히 권장합니다.
