export const ES_TEXTS = {
    ...
    welcomeNoteSettingsToggleDesc: 'Cuando está habilitado, el plugin crea <wikiFolder>/Welcome.md la primera vez que el plugin se carga y la carpeta wiki falta o está vacía. La nota es editable — rellena tu foco de dominio, elige 2-3 notas fuente para ingerir, y el grafo de enlaces se siembra automáticamente. La nota se traduce automáticamente a tu idioma wiki mediante el LLM (fallback a inglés si el LLM no está configurado). Usa el comando de paleta "Recrear nota Welcome Wiki" para regenerar bajo demanda. Desactiva para nunca crear automáticamente.',
