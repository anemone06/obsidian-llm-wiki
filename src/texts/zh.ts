export const ZH_TEXTS = {
    ...
    welcomeNoteSettingsToggleDesc: '启用时，插件在加载时若检测到 wiki 文件夹缺失或为空，会创建 <wikiFolder>/Welcome.md。该笔记可编辑 — 填写你的领域聚焦，挑选 2-3 条源笔记进行摄取，链接图便会自动播种。笔记会通过 LLM 自动翻译为你的 wiki 语言（若 LLM 未配置则回退为英文）。使用命令面板中的"重新创建 Wiki Welcome note"按需重新生成。禁用后插件不再自动创建。',
