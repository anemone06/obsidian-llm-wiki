export const JA_TEXTS = {
    ...
    welcomeNoteSettingsToggleDesc: '有効にすると、プラグイン起動時に wiki フォルダーが見つからないか空の場合、<wikiFolder>/Welcome.md を作成します。ノートは編集可能 — ドメインフォーカスを記入し、2-3 個のソースノートを取り込めば、リンクグラフが自動的にシードされます。ノートは LLM 経由で wiki 言語に自動翻訳されます（LLM 未設定時は英語にフォールバック）。コマンドパレットの「Wiki Welcome note を再作成」で必要に応じて再生成できます。無効にすると自動作成しません。',
